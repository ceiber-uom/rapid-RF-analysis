...
cw = floor(log10(100))+1;
fprintf(['Pass ' ' '*ones(1,cw)]);
format_str = [8*ones(1,cw) '%-' num2str(cw) 'd'];

% For each channel of the wave
for cc = 1:size(wave,1)
    fprintf(format_str,cc)
...
%    spectrum to give the EMG signal with reduced interference.        
        filtWave(mod(tt/step,nStep)+1,tt+1:tt+m) = real(ifft(spect));
    end
