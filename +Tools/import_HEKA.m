...
defaults = {'extractSpikes',    1; ...
            'removeLineNoise',  1; ... 
            'saveDIR',          [pwd  filesep 'Data' filesep]; ...
            'saveMAT',          1; ...
            'saveXML',          1};
find_options = cellfun(@isstruct,varargin);        
if any(find_options), options = varargin{find(find_options,1)}; end
        
for ii = 1:size(defaults,1)
    if ~isfield(options, defaults{ii,1}),
        options.(defaults{ii,1}) = defaults{ii,2};
    end
end
clear ii defaults find_options
